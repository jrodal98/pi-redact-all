...
const ENV_SECRET_PATTERN = new RegExp(`(?:^|[^A-Za-z0-9_])(?:${SECRET_FIELD_NAMES.join("|")})\\s*=\\s*["']?([^"'\\s]{8,})["']?`, "gi");
const INI_SECRET_PATTERN = new RegExp(`^\\s*(?:${SECRET_FIELD_NAMES.join("|")})\\s*[=:]\\s*(.+)$`, "gim");
const YAML_SECRET_PATTERN = new RegExp(`^\\s*(?:${SECRET_FIELD_NAMES.join("|")}):\\s*(.+)$`, "gim");
