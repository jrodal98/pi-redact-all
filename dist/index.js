...
function formatBlocklistReport() {
    ...
    if (literalList.length === 0)
        ...
        if (literalList.length > 20)
            lines.push(`- … and ${literalList.length - 20} more (use /redact-all-config for raw)`);
    }
    lines.push(``);
    ...
    if (envList.length === 0)
        ...
        for (const name of envList) {
            ...
            if (val)
                lines.push(`- \`${name}\` → \`${maskValue(val)}\` ✓ resolved (${val.length} chars)`);
            else
