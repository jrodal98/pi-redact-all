...
function formatBlocklistReport() {
    ...
    if (regexList.length === 0)
        lines.push(`(none) — add to pi-redact-all.json: "blocklistRegex": ["MY_SECRET_\\\\d+"]`);
    else {
    ...
    if (manualLiterals.length === 0)
        ...
        if (manualLiterals.length > 20)
            lines.push(`- … and ${manualLiterals.length - 20} more masked values`);
    }
    lines.push(``);
    lines.push(`### blocklistEnv (${envList.length} configured, ${resolvedEnv.size} resolved)`);
    if (envList.length === 0)
        lines.push(`(none) — e.g. "blocklistEnv": ["MY_SECRET_TOKEN"]`);
    else {
        for (const name of envList) {
    ...
    if (dotenvDebug) {
        ...
        if (dotenvDebug.loadedFiles.length === 0)
            lines.push(`  (none)`);
        else {
            for (const file of dotenvDebug.loadedFiles) {
                ...
                if (file.skippedKeys.length)
                    lines.push(`    - skipped keys: ${file.skippedKeys.join(", ")}`);
            }
        }
        if (dotenvDebug.skippedExampleFiles.length)
            lines.push(`- skipped example files: ${dotenvDebug.skippedExampleFiles.map((p) => `\`${p}\``).join(", ")}`);
