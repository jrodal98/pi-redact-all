...
let lastDotenvDebug = null;
let lastConfigPaths = { global: null, project: null, usedProject: null };
...
function mergeArrays(base, user) {
    if (!user)
        return base;
    if (!base.length)
        return user;
...
function applyDotenvBlocklist(config) {
...
function discoverDotenvFiles(strategies) {
    const out = [];
        ...
        if (strat === "gitroot" || strat === "git_root" || strat === "git-root") {
            ...
            for (const ex of examples) {
                ...
                if (base.endsWith(".example"))
                    envName = base.slice(0, -8);
                else if (base.endsWith(".sample"))
                    envName = base.slice(0, -7);
                else if (base.endsWith(".template"))
                    envName = base.slice(0, -9);
...
function discoverViaGitLsFiles(root) {
    try {
...
function findTrackedEnvExampleFiles(root) {
    try {
        const out = execSync("git ls-files -z --cached --others --exclude-standard", { encoding: "buffer", cwd: root, timeout: 2000 });
        const files = out.toString("utf-8").split("\0").filter(Boolean);
        return files
        ...
        try {
            ...
            function walk(dir) {
                ...
                for (const ent of ents) {
                    ...
                    if (ent.isDirectory())
                        walk(full);
                    else if (ent.name.endsWith(".env.example") || ent.name.endsWith(".env.sample"))
                        res.push(full);
                }
            }
            walk(root);
...
function loadDotenvValues(patterns, excludeKeys) {
    const { values } = loadDotenvValuesWithDebug(patterns, excludeKeys);
    return values;
}
function loadDotenvValuesWithDebug(patterns, excludeKeys) {
    const out = [];
...
function isDotenvExampleFile(file) {
    const base = basename(file).toLowerCase();
    return base.endsWith(".example") || base.endsWith(".sample") || base.endsWith(".template") || base === ".env.example" || base === ".env.sample";
}
function isDotenvPlaceholderValue(val) {
    const lower = val.toLowerCase();
    if (["", "example", "placeholder", "changeme", "your_value_here", "xxx", "***"].includes(lower))
        return true;
    if (/^\$\{.*\}$/.test(val))
        return true;
    return false;
...
function expandDotenvPattern(pattern) {
    ...
    try {
        ...
        if (typeof fs.globSync === "function") {
            ...
            return fs.globSync(absPat).map((p) => resolve(p));
        }
    }
    catch { }
    const absPat = isAbsolute(pattern) ? pattern : join(process.cwd(), pattern);
...
function expandGlobFallback(pattern) {
    ...
    if (results.length === 0)
        return [resolve(pattern.replace(/\*/g, ""))];
    return results;
}
